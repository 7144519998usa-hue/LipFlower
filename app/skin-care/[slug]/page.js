...
export async function generateMetadata({ params }) {
  ...
  return buildBeautyMetadata({
    ...
    canonicalPath: `/skin-care/${page.slug}`,
  });
}

export default async function SkinCareLandingPage({ params }) {
  const { slug } = await params;
  const page = getBeautyLandingPageForCategory(categoryPath, slug);
  if (!page) {
    notFound();
  ...
  return <BeautyLandingRoutePage eyebrow="Skin care route" page={page} />;
}
